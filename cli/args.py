...
def create_parser() -> argparse.ArgumentParser:
    ...
    # D11/Venus flags
    _add_d11_args(parser)

    # D12/Mercury flags
    _add_d12_args(parser)

    # Mercury thermal flags
    _add_mercury_args(parser)

    # TEE hardening flags
    _add_tee_args(parser)

    # Turbulent CFD flags
    _add_cfd_turbulent_args(parser)
...
def _add_d11_args(parser: argparse.ArgumentParser) -> None:
    """Add D11/Venus arguments."""
    parser.add_argument(
    ...
    parser.add_argument(
        "--d11_venus_hybrid", action="store_true", help="Run integrated D11+Venus"
    )
    parser.add_argument(
        "--venus_info", action="store_true", help="Show Venus acid-cloud configuration"
    )
    parser.add_argument(
        "--venus_config", action="store_true", help="Show Venus config from spec"
    )
    parser.add_argument(
        "--venus_simulate", action="store_true", help="Run Venus acid-cloud simulation"
    )
    parser.add_argument(
        "--venus_duration",
        type=int,
        default=24,
        help="Venus simulation duration in hours (default: 24)",
    )
    parser.add_argument(
        "--venus_altitude",
        type=float,
...
def _add_d12_args(parser: argparse.ArgumentParser) -> None:
    """Add D12/Mercury arguments."""
    parser.add_argument(
    ...
    parser.add_argument(
        "--d12_mercury_hybrid", action="store_true", help="Run integrated D12+Mercury"
    )


def _add_mercury_args(parser: argparse.ArgumentParser) -> None:
    """Add Mercury thermal arguments."""
    parser.add_argument(
        "--mercury_info", action="store_true", help="Show Mercury thermal configuration"
    )
    parser.add_argument(
        "--mercury_config", action="store_true", help="Show Mercury config from spec"
    )
    parser.add_argument(
        "--mercury_simulate", action="store_true", help="Run Mercury thermal simulation"
    )
    parser.add_argument(
        "--mercury_zone",
        type=str,
        default="terminator",
        help="Mercury operation zone (terminator, nightside, polar_crater)",
    )
    parser.add_argument(
        "--mercury_duration",
        type=int,
        default=24,
        help="Mercury simulation duration in hours (default: 24)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="inconel_718",
        help="Alloy type (inconel_718, haynes_230, tungsten_rhenium)",
    )
    parser.add_argument(
        "--mercury_cycling", action="store_true", help="Run thermal cycling test"
    )
    parser.add_argument(
        "--mercury_cycles",
        type=int,
        default=100,
        help="Number of thermal cycles (default: 100)",
    )


def _add_tee_args(parser: argparse.ArgumentParser) -> None:
    """Add TEE hardening arguments."""
    parser.add_argument(
        "--tee_info", action="store_true", help="Show TEE hardening configuration"
    )
    parser.add_argument(
        "--tee_config", action="store_true", help="Show TEE config from spec"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="TEE memory in MB (default: 256)",
    )
    parser.add_argument(
        "--tee_audit", action="store_true", help="Run full TEE side-channel audit"
    )
    parser.add_argument(
        "--tee_timing", action="store_true", help="Test timing defense"
    )
    parser.add_argument(
        "--tee_power", action="store_true", help="Test power balancing defense"
    )
    parser.add_argument(
        "--tee_cache", action="store_true", help="Test cache partition defense"
    )
    parser.add_argument(
        "--tee_branch", action="store_true", help="Test branch obfuscation defense"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--tee_overhead", action="store_true", help="Measure TEE overhead"
    )


def _add_cfd_turbulent_args(parser: argparse.ArgumentParser) -> None:
    """Add turbulent CFD arguments."""
    parser.add_argument(
        "--turbulent_info", action="store_true", help="Show turbulent CFD configuration"
    )
    parser.add_argument(
        "--turbulent_config", action="store_true", help="Show turbulent config from spec"
    )
    parser.add_argument(
        "--turbulent_simulate", action="store_true", help="Run turbulent flow simulation"
    )
    parser.add_argument(
        "--turbulent_reynolds",
        type=float,
    ...
    parser.add_argument(
        "--turbulent_storm", action="store_true", help="Run turbulent dust storm simulation"
    )
    parser.add_argument(
        "--turbulent_intensity",
        type=float,
        default=0.8,
        help="Storm intensity 0-1 (default: 0.8)",
    )
    parser.add_argument(
        "--turbulent_validate", action="store_true", help="Run turbulent validation"
    )
