...
class TestComputeYearsToThreshold:
    ...
    def test_high_multiplier_fast_years(self):
        ...
        years_low, years_high = compute_years_to_threshold(
            annual_multiplier=2.75, current_capability=1000, threshold=1_000_000
        )
    ...
    def test_low_multiplier_slow_years(self):
        ...
        years_low, years_high = compute_years_to_threshold(
            annual_multiplier=1.1, current_capability=1000, threshold=1_000_000
        )
    ...
    def test_no_growth_infinite(self):
        ...
        years_low, years_high = compute_years_to_threshold(
            annual_multiplier=1.0, current_capability=1000, threshold=1_000_000
        )
    ...
    def test_already_at_threshold(self):
        ...
        years_low, years_high = compute_years_to_threshold(
            annual_multiplier=2.0, current_capability=1_500_000, threshold=1_000_000
        )
...
class TestLatencyPenalty:
    ...
    def test_tau_penalty_linear_interpolation(self):
        ...
        assert abs(penalty_mid - expected_mid) < 0.01, (
            f"Expected ~{expected_mid}, got {penalty_mid}"
        )
...
class TestTimelineWithLatency:
    ...
    def test_milestone_early_delayed_mars(self):
        ...
        assert (
            result_mars["cycles_to_10k_person_eq"]
            > result_earth["cycles_to_10k_person_eq"]
...
class TestSovereigntyTimeline:
    ...
    def test_sovereignty_timeline_trajectory_grows(self):
        ...
        for i in range(1, len(traj)):
            assert traj[i] >= traj[i - 1], (
                f"Trajectory should grow: {traj[i - 1]} -> {traj[i]}"
            )
    ...
    def test_sovereignty_timeline_custom_params(self):
        ...
        assert (
            result_higher_c["cycles_to_10k_person_eq"]
            <= result_default["cycles_to_10k_person_eq"]
...
class TestReceiptMitigation:
    ...
    def test_timeline_receipt_mitigation(self):
        ...
        assert (
            result_yes["cycles_to_10k_person_eq"] < result_no["cycles_to_10k_person_eq"]
    ...
    def test_mitigation_vs_unmitigated(self):
        ...
        result_mitigated = sovereignty_timeline(
            50, 1.8, 1.69, 1200, receipt_integrity=0.9
        )
        result_unmitigated = sovereignty_timeline(
            50, 1.8, 1.69, 1200, receipt_integrity=0.0
        )
    ...
    def test_acceleration_8_15_cycles(self):
        ...
        cycles_saved = (
            result_unmitigated["cycles_to_10k_person_eq"]
            - result_mitigated["cycles_to_10k_person_eq"]
        )

        # Should save cycles (may vary based on model parameters)
        assert cycles_saved > 0, (
            f"Should save cycles with receipts, saved: {cycles_saved}"
        )
    ...
    def test_earth_unaffected_by_receipts(self):
        ...
        result_earth_receipts = sovereignty_timeline(
            50, 1.8, 1.69, 0, receipt_integrity=0.9
        )

        # Both should have same effective_alpha (no latency penalty to mitigate)
        assert (
            result_earth["effective_alpha"] == result_earth_receipts["effective_alpha"]
        )

        # Same cycles to milestone
        assert (
            result_earth["cycles_to_10k_person_eq"]
            == result_earth_receipts["cycles_to_10k_person_eq"]
        )
